    ...
    <div className="flex-1 p-8">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-zinc-400 mt-2">View and manage users in the system.</p>
        </div>
      </div>
            ...
            {users.map((user: any) => (
              <tr key={user.id} className="hover:bg-zinc-800/50">
                ...
                <td className="px-6 py-4">
                  <span className={`px-2.5 py-1 text-xs rounded-full border ${user.role === 'ADMIN' ? 'bg-indigo-500/10 text-indigo-400 border-indigo-500/20' : 'bg-zinc-800 text-zinc-300 border-zinc-700'}`}>
                    {user.role}
                  </span>
                </td>
                <td className="px-6 py-4">
